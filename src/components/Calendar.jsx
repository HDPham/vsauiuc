...
function Calendar(props) {
  ...
  const calendarDates = () => {
    ...
    for (let i = 1; i <= 7 * Math.ceil((numOfBlanks + numOfDays) / 7); i++) {
      if (i <= numOfBlanks || i > numOfBlanks + numOfDays)
        ...
        if (
          pastCount < props.pastEvents.length &&
          i - numOfBlanks >= props.pastEvents[pastCount].startDate.getDate() &&
          i - numOfBlanks <= props.pastEvents[pastCount].endDate.getDate()
        ) {
          cols.push(
            <Col
              key={i}
              className="border w-auto"
              style={{ flexBasis: 'auto', backgroundColor: 'coral' }}
            >
              <div className="position-relative" style={{ paddingTop: '100%' }}>
                ...
                <div
                  className="d-none d-md-block position-absolute w-100 font-weight-bold"
                  style={{ top: '50%', transform: 'translateY(-50%)' }}
                >
                  {props.pastEvents[pastCount].name}
                </div>
              </div>
            </Col>
          );
          ...
          upcCount < props.upcEvents.length &&
          i - numOfBlanks >= props.upcEvents[upcCount].startDate.getDate() &&
          i - numOfBlanks <= props.upcEvents[upcCount].endDate.getDate()
        ) {
          cols.push(
            <Col
              key={i}
  ...
  return (
    <section className="text-center">
      <h3>
        {props.currDate.toLocaleDateString(undefined, {
          month: 'long',
          year: 'numeric',
        })}
      </h3>
